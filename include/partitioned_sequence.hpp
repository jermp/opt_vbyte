...
struct partitioned_sequence {
    ...
    typedef partitioned_sequence_enumerator<base_sequence_type> enumerator;
        ...
        if (partitions == 1) {
            ...
            for (size_t p = 0; p < partition.size(); ++p) {
                ...
                base_sequence_type::write(
                    bv_sequences, cur_partition.begin(),
                    cur_partition.back() + 1,
            ...
            compact_elias_fano::write(bv_sizes, partition.begin(), n,
                                      partitions - 1, params);
...
private:
    ...
    static std::vector<uint32_t> compute_partition(
        Iterator begin, uint64_t universe, uint64_t n,
        global_parameters const& params, configuration const& conf) {
        std::vector<uint32_t> partition;

        auto cost_fun = [&](auto begin, uint64_t universe, uint64_t n) {
            return base_sequence_type::bitsize(begin, params, universe, n) +
                   conf.fix_cost;
        };

        const size_t superblock_bound =
            conf.eps3 != 0 ? size_t(conf.fix_cost / conf.eps3) : n;
        ...
        task_region(*conf.executor, [&](task_region_handle& thr) {
            ...
            while (superblock_pos < n) {
                size_t superblock_size =
                    std::min<size_t>(superblock_bound, n - superblock_pos);
                ...
                auto superblock_last =
                    std::next(superblock_begin, superblock_size - 1);
                ...
                thr.run([=, &cost_fun, &conf, &superblock_partition] {
                    optimal_partition<Encoder> opt(
                        superblock_begin, superblock_base, superblock_universe,
                        superblock_size, cost_fun, conf.eps1, conf.eps2, params,
                        conf.fix_cost);

                    superblock_partition.reserve(opt.partition.size());
                    for (auto& endpoint : opt.partition) {
                        superblock_partition.push_back(superblock_pos +
                                                       endpoint);
                    }
                });
        ...
        for (const auto& superblock_partition : superblock_partitions) {
            partition.insert(partition.end(), superblock_partition.begin(),
