...
public:
    ...
    virtual std::vector<uint32_t> compress(const std::vector<uint32_t>& data) {
        std::vector<uint32_t> compresseddata(
            data.size() * 2 + 1024);  // allocate plenty of memory
     ...
     * others need to uncompress it all to know how data there is to
     * uncompress... So it useful to have a hint (expected_uncompressed_size)
     * that tells how much data there will be to uncompress. Otherwise, the code
     * will try to guess, but the result is uncertain and inefficient. You
     * really ought to keep track of how many symbols you had compressed.
     *
...
public:
    ...
    VarIntG8IU() {
        ...
        for (int desc = 0; desc <= 255; desc++) {
            ...
            vecmask[desc][1] = _mm_lddqu_si128(
                reinterpret_cast<__m128i const*>(mask[desc] + 16));
        }
    }
    ...
    const uint32_t* decodeArray(const uint32_t* in, const size_t length,
        ...
        while (srclength >= 9) {
            ...
            if (readSize > 4) {
                ...
                _mm_storeu_si128(reinterpret_cast<__m128i*>(buff + 16),
                                 result2);
            }
        ...
        return reinterpret_cast<uint32_t*>(
            (reinterpret_cast<uintptr_t>(src) + 3) & ~3);
    }
