...
struct compact_ranked_bitvector {
    ...
    static void write(succinct::bit_vector_builder& bvb, Iterator begin,
        ...
        auto set_rank1_samples = [&](uint64_t begin, uint64_t end,
            ...
            for (uint64_t sample =
                ...
                if (!sample)
                    continue;
                offset = of.rank1_samples_offset +
                         (sample - 1) * of.rank1_sample_size;
    ...
    public:
        ...
        value_type move(uint64_t position) {
            ...
            if (DS2I_LIKELY(position > m_position &&
    ...
    private:
        ...
        value_type DS2I_NOINLINE slow_next_geq(uint64_t lower_bound) {
            ...
            uint64_t word = (m_bv->data()[begin_word] >> begin_shift)
                            << begin_shift;
        ...
        inline uint64_t pointer(uint64_t offset, uint64_t i,
                                uint64_t size) const {
            if (i == 0) {
                return 0;
            } else {
        ...
        // int bitmap_decode_avx2(uint64_t const* bitmap, size_t
        // size_in_64bit_words, uint32_t* out) {
        ...
        //         } else {
        //             for (int k = 0; k < 4; ++k) { // process 2 bytes of data
        //             at a time
        //                 uint8_t byteA = (uint8_t) w;
        //                 uint8_t byteB = (uint8_t)(w >> 8);
        //                 w >>= 16;
        //                 __m256i vecA = _mm256_load_si256((const __m256i *)
        //                 vecDecodeTable[byteA]);
        //                 __m256i vecB = _mm256_load_si256((const __m256i *)
        //                 vecDecodeTable[byteB]); uint8_t advanceA =
        //                 lengthTable[byteA]; uint8_t advanceB =
        //                 lengthTable[byteB]; vecA = _mm256_add_epi32(baseVec,
        //                 vecA); baseVec = _mm256_add_epi32(baseVec, add8);
