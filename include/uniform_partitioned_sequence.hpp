...
struct uniform_partitioned_sequence {
    ...
    typedef uniform_partitioned_sequence_enumerator<base_sequence_type,
                                                    UpperBounds>
        enumerator;
    ...
    static void write(succinct::bit_vector_builder& bvb, Iterator begin,
        ...
        if (partitions == 1) {
            ...
            for (size_t p = 0; p < partitions; ++p) {
                cur_partition.clear();
                uint64_t value = 0;
                for (; cur_i < ((p + 1) * partition_size) and cur_i < n;
                     ++cur_i, ++it) {
                    value = *it;
            ...
            UpperBounds::write(bv_upper_bounds, upper_bounds.begin(), universe,
                               partitions + 1, params);
