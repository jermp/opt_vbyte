...
namespace ds2i {
    ...
    struct uniform_partitioned_sequence_enumerator {
            ...
            , m_bv(&bv)
        {
            ...
            if (m_partitions == 1) {
                ...
                cur_offset +=
                    typename UpperBounds::offsets(0, universe, m_partitions + 1, *m_params).end;
        ...
        pv_type DS2I_ALWAYSINLINE move(uint64_t position) {
            ...
            if (m_position >= m_cur_begin and m_position < m_cur_end) {
                m_value = m_cur_base + m_partition_enumerator.move(m_position - m_cur_begin).second;
                return value();
            }
    ...
    private:
        ...
        void switch_partition(uint64_t partition)
        {
            ...
            uint64_t endpoint =
                partition ? m_bv->get_bits(m_endpoints_offset + (partition - 1) * m_endpoint_bits,
                                           m_endpoint_bits)
                          : 0;
            ...
            m_cur_base = m_upper_bounds_enumerator.prev_value() + (partition ? 1 : 0);
