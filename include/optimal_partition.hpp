...
struct optimal_partition {
    ...
    struct cost_window {
        ...
        posting_type min_p =
            0;  // element that preceeds the first element of the window
        posting_type max_p = 0;

        cost_t cost_upper_bound;  // max cost for this window
        cost_t third_cost =
            0;  // cost for this window as encoded with third compressor (not
                // all_ones, nor ranked_bitvector)

        cost_window(ForwardIterator begin, posting_type base,
                    cost_t cost_upper_bound, global_parameters const& params)
            : start_it(begin)
            , end_it(begin)
            , min_p(base)
            , max_p(base)
        ...
        void advance_start() {
            uint64_t posting_cost = Encoder::posting_cost(*start_it, min_p - 1);
        ...
        void advance_end() {
            uint64_t posting_cost = Encoder::posting_cost(*end_it, max_p);
            third_cost += posting_cost;
            max_p = *end_it;
            ++end;
            ++end_it;
    ...
    optimal_partition(ForwardIterator begin, posting_type base,
                      posting_type universe, uint64_t size,
        ...
        for (posting_type i = 0; i < size; i++) {
            ...
            for (auto& window : windows) {
                ...
                while (true) {
                    // window_cost = cost_fun(window.start_it,
                    // window.universe(), window.size());
                    ...
                    if (window_cost >= window.cost_upper_bound)
                        break;  // ignore this for quadratic solution
                    window.advance_end();
                }
    ...
    //                   posting_type base, posting_type universe, uint64_t
    //                   size, CostFunction cost_fun, double eps1, double eps2,
    //                   global_parameters const& params, uint64_t fix_cost)
    // {
    //     (void) eps1;
