...
struct partitioned_sequence_enumerator {
    ...
    partitioned_sequence_enumerator(succinct::bit_vector const& bv,
                                    ...
                                    uint64_t n, global_parameters& params)
        : m_params(&params)
        , m_size(n)
        , m_universe(universe)
        , m_position(0)
    ...
    //         m_partition_enum.access_delta(m_position - m_cur_begin).second;
    //         return pv_type(m_position, val);
    //     }
    ...
    //     auto size_it = m_sizes.next_geq(m_position + 1);
    //     switch_partition(size_it.first);
    //     uint64_t val = m_cur_base + m_partition_enum.access_delta(m_position
    //     - m_cur_begin).second; return pv_type(m_position, val);
    // }
    ...
    // void next_partition()
    // {
    //     uint64_t endpoint = m_bv->get_word56(m_endpoints_offset +
    //     m_cur_partition * m_endpoint_bits)
    //              & ((uint64_t(1) << m_endpoint_bits) - 1);
