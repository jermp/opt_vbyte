...
public:
    ...
    private:
        ...
        struct sequence_adder : semiasync_queue::job {
            sequence_adder(builder& b, Iterator begin, uint64_t last_element,
                           uint64_t n)
            ...
            virtual void commit() {
                bits.append_bits(0, 8 - ((bits.size()) % 8));  // FIX THIS
    ...
    enumerator_type operator[](size_t i) const {
        ...
        return enumerator_type(m_sequences.bits(), it.position(),
                               (uint64_t(1) << universe_bits) + 1, n, m_params);
    }
    ...
    void map(Visitor& visit) {
        visit(m_params, "m_params")(m_size, "m_size")(m_sequences,
                                                      "m_sequences");
    }
