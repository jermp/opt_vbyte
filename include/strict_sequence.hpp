...
struct strict_sequence {
    ...
    static DS2I_FLATTEN_FUNC uint64_t bitsize(Iterator begin,
        ...
        uint64_t ef_cost =
            strict_elias_fano::bitsize(sparams, universe, n) + type_bits;
        if (ef_cost < best_cost) {
            best_cost = ef_cost;
        }

        uint64_t rb_cost =
            compact_ranked_bitvector::bitsize(sparams, universe, n) + type_bits;
    ...
    static void write(succinct::bit_vector_builder& bvb, Iterator begin,
        ...
        if (best_cost) {
            ...
            uint64_t rb_cost =
                compact_ranked_bitvector::bitsize(sparams, universe, n) +
                type_bits;
            if (rb_cost < best_cost) {
                best_cost = rb_cost;
        ...
        switch (best_type) {
            case elias_fano:
                strict_elias_fano::write(bvb, begin, universe, n, sparams);
                break;
            case ranked_bitvector:
                ...
                break;
            case all_ones:
                all_ones_sequence::write(bvb, begin, universe, n, sparams);
                break;
            default:
                assert(false);
        }
    ...
    public:
        ...
        enumerator(succinct::bit_vector const& bv, uint64_t offset,
            ...
            switch (m_type) {
                case elias_fano:
                    m_ef_enumerator = strict_elias_fano::enumerator(
